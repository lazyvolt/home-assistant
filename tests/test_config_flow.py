...
async def test_user_step_success_single_peblar(hass, mock_peblar_entry):
    """Auth succeeds and single Peblar entry → entry created without peblar step."""
...
async def test_user_step_multiple_peblar_shows_selection(hass):
    ...
    with patch(
        ...
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"cloud_url": MOCK_CLOUD_URL, "edge_name": "HA", "email": "u@e.com", "password": "p"},
        )
...
async def test_user_step_invalid_auth_shows_error(hass, mock_peblar_entry):
    ...
    with patch(
        ...
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"cloud_url": MOCK_CLOUD_URL, "edge_name": "HA", "email": "bad@e.com", "password": "wrong"},
        )
